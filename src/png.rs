...
impl Png {
    ...
    pub const STANDARD_HEADER: [u8; 8] = [137, 80, 78, 71, 13, 10, 26, 10];
		...
		pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Vec<u8>> {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk = self.chunks().iter().find(|c| c.chunk_type().to_string()==chunk_type);
				return chunk;
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunk_bytes: Vec<u8> = self.chunks().iter().flat_map(|c| c.as_bytes()).collect();
				return self.header().iter().chain(chunk_bytes.iter()).copied().collect()
    }
}
